...
import { useResourcesHub } from "@/hooks/useResourcesHub";
        ...
        <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 max-w-5xl">
          <div className="min-w-0">
            ...
            <h2 className="mt-3 text-3xl sm:text-4xl md:text-5xl font-display text-gradient">
              The Red Berry Resources Hub
            </h2>

            <p className="mt-4 text-foreground/65 text-base sm:text-lg max-w-2xl">
              Strategic resources for founders, investors,
              families, and entrepreneurs building in the GCC.
            </p>
          </div>
        ...
        <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          ...
          {!loading &&
            ...
            items.map((item, index) => (
              <Link
                key={item.slug}
                to={`/resources-hub/${item.slug}`}
                className="group block"
              >
                <article className="h-full p-5 sm:p-6 md:p-7 rounded-2xl glass hover:shadow-[var(--shadow-lift)] transition-all">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-[10px] tracking-[0.2em] uppercase text-foreground/45">
                      Pillar
                    </div>
                  </div>

                  <h3 className="mt-5 text-lg sm:text-xl font-display text-foreground leading-tight">
                    {item.title}
                  </h3>
